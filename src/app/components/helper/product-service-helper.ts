...
export class ProductServiceHelper {
    ...
    protected addProductToStorage(id: number) {
        ...
        if (storage != null && storage != '') {
            ...
            localStorage.setItem('aidup_cart', JSON.stringify(items));
            this.storageSub.next(JSON.stringify(storage));
        } else {
            localStorage.setItem('aidup_cart', JSON.stringify([id.toString()]));
            this.storageSub.next(JSON.stringify(storage));
        }
    ...
    protected getProductsFromStorage() {
        return localStorage.getItem('aidup_cart') != null ? localStorage.getItem('aidup_cart') : '';
    }
    ...
    protected addWishlistProductToStorage(id: number) {
        ...
        if (storage != null && storage != '') {
            items = JSON.parse(storage);
            items.push(id.toString());
            localStorage.setItem('aidup_wishlist', JSON.stringify(items));
            this.storageSub.next(JSON.stringify(storage));
        } else {
            localStorage.setItem('aidup_wishlist', JSON.stringify([id.toString()]));
            this.storageSub.next(JSON.stringify(storage));
        }
    ...
    protected getWishlistProductsFromStorage() {
        return localStorage.getItem('aidup_wishlist') != null ? localStorage.getItem('aidup_wishlist') : '';
    }
